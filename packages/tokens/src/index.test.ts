...
describe("figma import bundle", () => {
  ...
  it("includes Badge, Tag, Alert, Switch, Tooltip, Popover, Divider, and Utility component tokens", () => {
    ...
    expect(light?.["Alert/color/info/bg"]).toBe("#003877")
    expect(light?.["Alert/color/info/border"]).toBe("#0059C2")
    expect(light?.["Alert/color/info/accent"]).toBe("#54A3F6")
    expect(light?.["Alert/color/warning/bg"]).toBe("#521D00")
    expect(light?.["Alert/color/danger/accent"]).toBe("#FF755E")

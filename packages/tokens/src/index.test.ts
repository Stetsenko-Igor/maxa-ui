...
describe("figma import bundle", () => {
  ...
  it("includes Badge, Tag, and Utility component tokens", () => {
    ...
    expect(light?.["Badge/fg-neutral"]).toBe("{Utility/text-gray}")
    expect(light?.["Badge/fg-info"]).toBe("{Utility/text-blue}")
    expect(light?.["Badge/fg-inverse"]).toBe("{Color modes/text/text-inverse}")
    expect(light?.["Badge/fg-violet"]).toBe("{Utility/text-violet}")

...
Implemented inventory: **35 components** (all with spec, implementation, docs, tests).
...
| Sheet / Slideout | Side panels, editor/tool settings, responsive menus | Missing | Slideout menus | Sheet, Drawer | P1 | Build after Dialog/Menu basics |
| Toast / Notification | Save/download/share feedback and async workflow status | Missing | Notifications, alerts | Sonner, Toast | P1 | Build after Dialog/Menu basics |
| Table / DataTable | Orders, admin/account lists, request lists likely need structured data | Table primitive shipped; DataTable pending | Tables | Table, Data Table | P1 | Primitive shipped; build DataTable only after product workflow spec |

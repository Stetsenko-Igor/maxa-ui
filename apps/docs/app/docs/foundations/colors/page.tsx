...
const NEUTRAL_USAGE: Record<string, { light: string[]; dark: string[] }> = {
  ...
  "--color-neutral-25": { light: [], dark: [] },
  "--color-neutral-50": { light: ["bg/page"], dark: [] },
  "--color-neutral-100": {
    light: ["border/tertiary", "bg/muted", "bg/disabled", "bg/neutral-subtle"],
    dark: [],
  },
  "--color-neutral-200": { light: ["border/secondary", "bg/neutral-muted"], dark: [] },
  "--color-neutral-300": {
    light: ["border/primary", "action/neutral", "bg/neutral-on-muted"],
    dark: [],
  },
  "--color-neutral-400": { light: ["action/neutral-hover"], dark: ["bg/neutral-strong"] },
  "--color-neutral-500": {
    light: ["text/disabled", "action/neutral-active"],
    dark: ["text/tertiary", "border/neutral-strong"],
  },
  "--color-neutral-600": { light: ["text/tertiary"], dark: ["action/neutral-active"] },
  "--color-neutral-700": {
    light: [],
    dark: ["border/primary", "text/disabled", "bg/neutral-on-muted"],
  },
  "--color-neutral-800": {
    light: ["text/secondary", "bg/neutral-strong"],
    dark: ["border/secondary", "bg/float", "bg/neutral-muted"],
  },
  "--color-neutral-900": {
    light: [],
    dark: ["bg/surface", "border/tertiary", "bg/neutral-subtle"],
  },
  "--color-neutral-925": { light: [], dark: ["bg/neutral-surface"] },
...
const USAGE: Record<string, string> = {
  ...
  "--color-text-on-color": "White labels on strong blue, green, and red actions in every theme.",
  "--color-fg-on-color": "White icons on strong colored actions in every theme.",
  "--color-text-link": "Accessible link text on regular surfaces.",
  "--color-text-link-hover": "Link text on hover.",
  "--color-text-link-active": "Link text while pressed.",
  "--color-fg-link": "Icon color paired with a link label.",
  "--color-fg-link-hover": "Link icon color on hover.",
  "--color-fg-link-active": "Link icon color while pressed.",
      ...
      {ANNOTATION_LABELS.map(({ name, labelY, targetY, chip }) => {
        ...
        return (
          <g key={name}>
            ...
            />
            {/* Target dot */}
            <circle cx={CX} cy={targetY} r="3" style={{ fill: "var(--color-border-secondary)" }} />
            ...
            {chip === "radius" && (
              <path
                d={`M ${LX + 24},${labelY + 20} L ${LX + 24},${labelY + 11} Q ${LX + 24},${labelY + 8} ${LX + 27},${labelY + 8} L ${LX + 36},${labelY + 8}`}
  ...
  return (
    ...
    >
      ...
      <DocsPageSection
        id="foreground"
        title="Foreground"
        description="Use foreground tokens for icons, SVG strokes, and non-text foreground elements. Use text tokens only for typography."
      >
      ...
      <DocsPageSection
        id="action"
        title="Action"
        description="Interactive backgrounds for default, hover, active, and subtle action states. These are component-state tokens, not general surface tokens."
      >
      ...
      <DocsPageSection
        id="background"
        title="Background"
        description="Surface elevation tokens plus status and intent backgrounds. Surface tokens describe where the layer sits; intent tokens describe the message or state."
      >
        <div style={swatchGridStyle}>
          {BG_GROUPS.map((s) => (
            <Swatch key={s.token} {...s} />
      ...
      <DocsPageSection
        id="control"
        title="Control"
        description="Idle and checked states for form controls: Checkbox, Radio, Toggle."
      >
        <div style={swatchGridStyle}>
          {CONTROL_GROUPS.map((s) => (
            <Swatch key={s.token} {...s} />
          ))}
        </div>
      </DocsPageSection>
      ...
      <DocsPageSection
        id="feedback"
        title="Feedback"
        description="Alert and feedback-surface roles: intent backgrounds, borders, accents, and action colors that keep the published Alert appearance stable."
      >
        <div style={swatchGridStyle}>
          {FEEDBACK_GROUPS.map((s) => (
            <Swatch key={s.token} {...s} />
          ))}
        </div>
      </DocsPageSection>
      ...
      <DocsPageSection
        id="text"
        title="Text"
        description="Typography-only color roles. Keep icon and SVG color on foreground tokens, even when the visual color matches."
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
